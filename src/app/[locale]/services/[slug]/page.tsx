...
export async function generateMetadata({
  ...
  return pageMetadata({
    ...
    description: clampDescription(
      `${s.shortDescription} ETCS delivers this scope across the Kingdom of Saudi Arabia.`
    ),
    keywords: [s.title, ...s.scope.slice(0, 4), ...s.sectors.slice(0, 3)],
    // Per-service card generated by the co-located opengraph-image route.
    ogImagePath: `/${locale}/services/${s.slug}/opengraph-image`,
    ogImageAlt: `${s.title} — ETCS`,
  });
}
...
export default async function ServiceDetailPage({
  ...
  const categoryLabel = tCat(s.category as never);
  const jsonLd = graph(
    webPageSchema({
      locale,
      path: `/services/${s.slug}`,
    ...
    serviceSchema(locale, s, categoryLabel)
  );
