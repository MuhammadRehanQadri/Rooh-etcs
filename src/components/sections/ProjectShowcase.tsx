...
function ProjectCard({ project, idx }: { project: Project; idx: number }) {
  const t = useTranslations();
  const inner = <ProjectCardContent project={project} idx={idx} t={t} />;
  if (project.featured) {
    return (
      <Link
        href={`/projects/${project.slug}` as never}
        className="block group relative h-full overflow-hidden rounded-2xl bg-navy-900 ring-1 ring-bone-200"
      >
        {inner}
      </Link>
    );
    ...
    <>
      ...
      <div className="absolute inset-x-0 bottom-0 p-6 sm:p-8 text-white">
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.22em] mb-3">
          <span
            className={`size-1.5 rounded-full ${
              project.status === "ongoing" ? "bg-gold-500 animate-pulse" : "bg-white/70"
            }`}
          />
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-white/65">
          ...
          {project.year && <span className="text-white/65">{project.year}</span>}
          {project.featured && (
            <span className="ms-auto text-gold-400 group-hover:underline">Read case study →</span>
          )}
        </div>
      </div>
